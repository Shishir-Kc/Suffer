...
const badgeMeta: Record<QuestType, { label: string; className: string; Icon: typeof Compass }> = {
...
export function QuestBadge({ type }: { type: QuestType }) {
  ...
  return <span className={`quest-badge ${className}`}><Icon size={14} aria-hidden="true" /> {label}</span>;
}
